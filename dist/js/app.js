(() => {
    ...
    function tabs() {
        ...
        function updateTabsContent(tabsBlock) {
            ...
            tabsContent.forEach(((content, index) => {
                if (tabsTitles[index].classList.contains("_tab-active")) {
                    if (animateDuration) _slideDown(content, animateDuration); else content.hidden = false;
                    setHash(`tab-${tabsIndex}-${index}`);
                } else if (animateDuration) _slideUp(content, animateDuration); else content.hidden = true;
            }));
        }
        function setTabsStatus(tabsBlock) {
            ...
            function isTabsAnamate(tabsBlock) {
                if (tabsBlock.hasAttribute("data-tabs-animate")) return tabsBlock.dataset.tabsAnimate > 0 ? Number(tabsBlock.dataset.tabsAnimate) : 700;
            }
    ...
    function menuInit() {
        if (document.querySelector(".main-header__menu")) document.addEventListener("click", (function(e) {
            if (bodyLockStatus && e.target.closest(".main-header__menu")) {
                bodyLockToggle();
    ...
    function initSliders() {
        ...
        if (document.querySelector(".regard__slider")) new swiper_core_Swiper(".regard__slider", {
            modules: [ Navigation ],
            observer: true,
            observeParents: true,
            slidesPerView: 4,
